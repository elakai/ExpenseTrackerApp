...
fun ChartScreen(navController: NavController, viewModel: ExpenseViewModel) {
    ...
    Scaffold(
        ...
        if (selectedMonth != null) {
            ...
            ChartContent(monthlyExpenses = monthlyExpenses, modifier = Modifier.padding(paddingValues))
        } else {
...
fun ChartContent(monthlyExpenses: List<Expense>, modifier: Modifier = Modifier) {
    ...
    AndroidView(
        ...
        update = { pieChart ->
            val categoryTotals = monthlyExpenses.fold(mutableMapOf<String, Double>()) { acc, expense ->
